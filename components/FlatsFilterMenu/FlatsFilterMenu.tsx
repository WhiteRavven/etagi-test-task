...
const getFilterChipLabel = (field: string, value: NumericRange | undefined) => {
    return `${DISPLAY_NAMES[field]} ${value?.min ? `от ${value.min} ` : ''}${value?.max ? `до ${value.max}` : ''}`;
}
...
export default function FlatsFilterMenu({
                                            ...
                                            { filter: FlatsFilter, onChange: (filterName: string, value: NumericRange | null) => void, onSubmit: () => void, onClear: () => void }) {
    ...
    return <>
        <Paper style={{padding: 24}}>
            <Box display="flex" flexDirection="column" gap="12px">
                <Box display="grid" gap="16px" gridTemplateColumns="repeat(auto-fit, 300px)">
                    {[...FLATS_FILTERS_BASE, ...(showExtraFilter ? FLATS_FILTERS_EXTRA : [])].map((field: string) =>
                        <Box key={field}>
                            <Box mb="4px">
                                <Typography variant="body1">{DISPLAY_NAMES[field]}</Typography>
                            </Box>
                            <NumericRangeSelect key={field}
                                                value={filter[field] ?? {}}
                ...
                {Object.keys(filter).length > 0 ? <Box display="flex" alignItems="center" gap="8px">
                    {Object.keys(filter).map((key, index) => <Chip key={index}
