...
export default function FloorLayoutViewer({selectedFlat, flats}: { selectedFlat: Flat, flats: Flat[] }) {
    ...
    const translateCoords = ([x, y]: number[]) => [
        Math.round(x * width),
        Math.round(y * height),
    ];
                ...
                <svg height={height} width={width} className={styles.polygonMap}>
                    {flats.map((flat: Flat) => {
                        ...
                        return (
                            <polygon
                                key={flat.id}
                                fill={flat.id === selectedFlat.id ? "green" : "blue"}
                                opacity={flat.id === selectedFlat.id ? "0.5" : "0"}
                                points={points}
